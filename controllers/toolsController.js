...
console.log(req.body);
    const { english_name, price, quantity, size, product_role, supplier_id } = req.body;
...
const deleteTools = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    // Update delete_status to false instead of deleting the product
    await Product.findByIdAndUpdate(product_id, { delete_status: false });

    res.status(200).json({
      message: "Tools updated successfully (delete_status set to false).",
      status: true,
    });
  } catch (error) {
...
const getAllTools = asyncHandler(async (req, res) => {
  ...
  try {
    const query = {
      $and: [
        { product_role: "tools" }, // Add the condition for product_role
        { delete_status: true }, // only fliter not deleted product
        {
          $or: [{ english_name: { $regex: search, $options: "i" } }],
        },
      ],
    ...
    const tools = await Product.find(query)
      .populate({
        path: "supplier_id",
        model: "User",
        select: "full_name email mobile profile_pic address", // Selecting required fields
      })
      .sort({ [sortBy]: order })
      .skip((page - 1) * limit)
      .limit(limit);
