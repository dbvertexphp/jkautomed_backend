...
const addFertilizer = asyncHandler(async (req, res, next) => {
  ...
  upload.array("product_images")(req, res, async (err) => {
    ...
    const { english_name, product_weight, price, quantity, category_name, product_role, supplier_id } = req.body;
  ...
  try {
    ...
    if (Array.isArray(imagePaths) && imagePaths.length > 0) {
      imagePaths.forEach((imagePath) => {
        if (imagePath) {
          const filePath = path.join(__dirname, "..", "uploads", "fertilizer", path.basename(imagePath));
...
const getAllFertilizer = asyncHandler(async (req, res) => {
  ...
  try {
    const query = {
      $and: [
        { product_role: "fertilizer" }, // Add the condition for product_role
        { delete_status: true }, // Filter for products where delete_status is true
        {
          $or: [{ english_name: { $regex: search, $options: "i" } }],
        },
      ],
