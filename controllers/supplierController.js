...
const deleteSupplierProduct = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    // Update delete_status instead of deleting the product
    product.delete_status = false;
    await product.save();

    res.status(200).json({
      message: "Product status updated to active (delete_status: false).",
      status: true,
    });
  } catch (error) {
    console.error("Error updating product status:", error.message);
...
const getProductsBySupplierId = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const products = await Product.find({ supplier_id, delete_status: true }).populate("supplier_id").skip(skip).limit(limit);
...
const updateOrderItemStatus = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const savedOrder = await Order.findOneAndUpdate(
      ...
      {
        arrayFilters: [{ "elem.supplier_id": supplier_id }], // This ensures all matching items are updated
        new: true,
      }
    );
        ...
        populate: [
          {
            ...
            model: "User",
            select: "full_name email mobile_number", // Adjust fields as needed
          },
          {
            path: "items.product_id", // Populate product details inside items array
            model: "Product",
...
const getFertilizerBySupplierId = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const products = await Product.find({ supplier_id, product_role: "fertilizer", active: true, delete_status: true });
...
const getToolsBySupplierId = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const products = await Product.find({ supplier_id, product_role: "tools", active: true, delete_status: true });
